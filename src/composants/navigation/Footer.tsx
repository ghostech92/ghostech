...
import { SITE_CONFIG } from "@/src/constantes/site-config";
      ...
      <div className="w-full bg-[#002C33] pt-48 pb-12 px-6 relative z-10">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-12 md:gap-8">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/formation" className="text-[#9ca3af] hover:text-white font-medium text-[15px] transition-colors">Formations</Link>
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-[#9ca3af] font-medium text-[15px]">{SITE_CONFIG.contact.address}</p>
            <a href={`mailto:${SITE_CONFIG.contact.email}`} className="text-[#9ca3af] hover:text-white font-medium text-[15px] transition-colors">
              {SITE_CONFIG.contact.email}
            </a>
            <a href={SITE_CONFIG.contact.phoneCall} className="text-[#9ca3af] hover:text-white font-medium text-[15px] transition-colors">
        ...
        <div className="max-w-7xl mx-auto mt-20 flex flex-col md:flex-row justify-between items-center md:items-end gap-6 relative">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 w-full pr-12 text-center sm:text-left">
            <div className="p-1.5 rounded-lg shadow-sm shrink-0">
              <img src="/logo1.svg" alt="Ghostech Logo" className="h-4 w-auto object-contain" />
            </div>
            <p className="text-white text-[11px] sm:text-[13px] md:text-sm leading-relaxed">
              Copyright: © {new Date().getFullYear()} <span className="text-[#357dab] font-bold">Ghostech Côte d'Ivoire</span> | Tous droits réservés.
              <span className="mx-2 text-slate-500 hidden sm:inline">|</span>
              <br className="sm:hidden" />
              <Link href="/createur" className="text-[#e49834] hover:underline font-semibold transition-colors mt-1 sm:mt-0 inline-block">
                 Conçu par Harding
              </Link>
            </p>
          </div>
